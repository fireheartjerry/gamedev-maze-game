...
from wall_generator import update_walls
...
def startEditor():
    ...
    configMenu[0].grid(row=1, column=0, padx=2, pady=10)
    configMenu[1].grid(row=1, column=1, pady=10)
    helpButton.grid(row=0, column=2, padx=5)
    printDataButton.grid(row=0, column=3, padx=5)
    updateDataButton.grid(row=0, column=4, padx=5)
...
updateDataButton = Button(toolFrame, text = "Update Wall Data", command = showData, **BTN_STLYLESHEET)
